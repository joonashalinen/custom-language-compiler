...
namespace MyLanguage {
    namespace TypeCheckers {
        ...
        TypeChecker::DTypeCheckContext* postCheckNormalBinaryOperator(
            ...
            if (
                valueTypes.at(0).toString() != "Any" &&
                valueTypes.at(1).toString() != "Any" &&
                valueTypes.at(0).toString() != valueTypes.at(1).toString()
            ) {
                auto errorMessage = (
                    std::string("The left value to binary operator '") + operatorName + 
                    "' was of type '" + valueTypes.at(0).toString() +
                    "' but the right value was of type '" + valueTypes.at(1).toString() + "'"
                );
                ...
                if (valueTypes.at(0).toString() != "Any" && !(acceptedTypesForOperator.contains(valueTypes.at(0).toString()))) {
                    throwTypeError(
                        expression,
                        std::string("The left-hand type '") + valueTypes.at(0).toString() + "' to binary operator '" + 
                        operatorName + "' is an invalid type"
                    );
                } else if (valueTypes.at(1).toString() != "Any" && !(acceptedTypesForOperator.contains(valueTypes.at(1).toString()))) {
                    throwTypeError(
                        expression,
                        std::string("The right-hand type '") + valueTypes.at(1).toString() + "' to binary operator '" + 
                        operatorName + "' is an invalid type"
                    );
                }
                context->typeStack.stack().push(Type{returnedTypes.at(operatorName)});
            }
            return context;
        }
        ...
        TypeChecker::DTypeCheckContext* postCheckVariableDeclaration(
            ...
            context->typeStack.stack().push(Type{"Unit"});
        ...
        TypeChecker::DTypeCheckContext* postCheckAssignment(
            ...
            auto leftHandType = context->typeSymbolTable.at(variableName).toString();
            ...
            auto valueType = context->typeStack.pop().toString();
        ...
        TypeChecker::DTypeCheckContext* postCheckUnaryOperator(
            ...
            auto invalidTypeError = (
                std::string("The type '") + valueType.toString() + 
            ...
            if (operatorName == "not" || operatorName == "-") {
                ...
                if (!(acceptedTypes.at(operatorName).contains(valueType.toString()))) {
                    throwTypeError(expression, invalidTypeError);
                }
           } else if (operatorName == "&") {
                valueType = Type{valueType + "*"};
            } else if (operatorName == "*") {
                if (valueType.toString().back() != '*') {
                    throwTypeError(expression, invalidTypeError);
                }
                valueType.pop_back();
            } else {
    ...
    void TypeChecker::check(TExpression root)
    {
        ...
        context.functionTypeSymbolTable.insert("print_int", TypeChecker::DFunctionType{{Type{"Int"}}, Type{"Unit"}});
        context.functionTypeSymbolTable.insert("print_bool", TypeChecker::DFunctionType{{Type{"Bool"}}, Type{"Unit"}});
        context.functionTypeSymbolTable.insert("read_int", TypeChecker::DFunctionType{{}, Type{"Int"}});
