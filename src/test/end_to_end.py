    ...
    for fileName in testFileNames:
        if fileName[0] != "_":
            ...
            with open(folder + "/" + fileName, encoding='utf-8') as file:
                ...
                contents = file.read()
                # Whether the test is expecting the program to produce an error.
                expectingCompilerError = "!expect-compiler-error!" in contents
                testSeparator = "!expect-compiler-error!" if expectingCompilerError else "!expect!"
                ...
                # If we are expecting an error from the compiler.
                if expectingCompilerError:
                    # Check if the compiler errors contain the expected error message.
                    if (not (expectedPrints in compilerResult.stderr)):
                        errorMessage = (
                    ...
                    # Else, we are expecting the compiler to produce no errors.
                    if compilerResult.stderr != "":
                        ...
                        # Check if the prints match with what we expect from running the compiled program.
